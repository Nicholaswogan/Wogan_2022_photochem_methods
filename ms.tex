...
\end{equation}
Applying a centered scheme to the diffusive fluxes follows the same procedure as we did with gas diffusion (Section \ref{sec:FV_particles}). To apply a first-order upwind scheme, we first recognize that falling particles means information is traveling from up to down in the model. Therefore, an upwind scheme approximates advective fluxes with a forward difference
